...
loginBtn.onclick = async () => {
  ...
  if (!email || !password) { loginErrorEl.textContent = "All fields are required"; return; }
  ...
  try {
    ...
    if (!res.ok) { loginErrorEl.textContent = data.message || "Login failed"; return; }
...
signupBtn.onclick = async () => {
  ...
  if (!username || !email || !password) { signupErrorEl.textContent = "All fields are required"; return; }
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) { signupErrorEl.textContent = data.message || "Signup failed"; return; }
    saveAuth(data.token, data.user);
    showApp();
  } catch {
...
async function loadProjects() {
  if (!isLoggedIn()) {
    projectsList.innerHTML = '<p class="empty-text">Login to see your projects</p>';
    return;
  }
  ...
  try {
    ...
    data.forEach(project => {
      ...
      div.className = "project-item" + (project._id === currentProjectId ? " active" : "");
...
createProjectBtn.onclick = async () => {
  if (!isLoggedIn()) { alert("Please login to create projects"); return; }
  ...
  try {
    const res  = await fetch("/api/projects", {
      ...
      body: JSON.stringify({ name: name.trim(), language: languageSelect.value, code: codeBox.value })
    });
    const data = await res.json();
...
saveBtn.onclick = async () => {
  if (!isLoggedIn())      { alert("Login to save projects"); return; }
